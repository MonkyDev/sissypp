<?php
	...
	$sql="SELECT alumnos.matricula AS alnMatricula,alumnos.nombre AS alnNombre,alumnos.paterno,alumnos.materno,alumnos.domicilio,alumnos.grupo,alumnos.sexo,alumnos.fnacimiento,alumnos.edad,alumnos.telefono,alumnos.edo,alumnos.pswd,alumnos.email,alumnos.carrera,servsocial.idservsocial,servsocial.no_cons,servsocial.anhio,servsocial.f_solicitud,servsocial.f_inicio,servsocial.f_fin,servsocial.horario,servsocial.programa,servsocial.nomEmp,servsocial.tipoEmp,servsocial.giroEmp,servsocial.dirEmp,servsocial.telEmp,servsocial.resEmp,servsocial.cargo,servsocial.edoEmp,servsocial.muniEmp,servsocial.matricula AS servMatri,programas.idprogramas,programas.descripcion AS prgDes,grupos.idgrupos,grupos.grado,grupos.grupo,grupos.turno,grupos.carr,turnos.idturnos,turnos.descripcion AS turnoDes,carreras.idcarreras,carreras.nombre AS carNombre, reports_sersocial.idreports_sersocial, reports_sersocial.folioServicio, reports_sersocial.actividades, reports_sersocial.objs_grales, reports_sersocial.objs_espe, reports_sersocial.departamento FROM alumnos INNER JOIN servsocial ON servsocial.matricula=alumnos.matricula INNER JOIN programas ON programas.idprogramas=servsocial.programa INNER JOIN grupos ON grupos.idgrupos=alumnos.grupo INNER JOIN turnos ON turnos.idturnos=grupos.turno INNER JOIN carreras ON carreras.idcarreras=alumnos.carrera INNER JOIN reports_sersocial  WHERE servsocial.matricula  = $mat";
	...
	$pdf->Ln(1);
	$pdf->Cell(0,1,'CARATULA DE PRESENTACION - REPORTE DE ACTIVIDADES'.$row2['tipo'], 0, 0, 'C');
	...
	$pdf->SetXY(1.5,8.3);
	$pdf->Cell(0,1,'1.2- Periodo del reporte presentado: ________________________________________________', 0, 1, 'J');
	$pdf->SetXY(8.5,8.3);
	...
	$pdf->SetXY(1.5,11.3);
	$pdf->Cell(0,1,'1.5- Empresa Asignada: __________________________________________________________', 0, 1, 'J');
	$pdf->SetXY(6.3,11.3);
	$pdf->Cell(0,1,utf8_decode($row['nomEmp']), 0, 1, 'J');
	
	$pdf->SetXY(1.5,12.3);
	$pdf->Cell(0,1,'1.6- Domicilio de la Empresa: ______________________________________________________', 0, 1, 'J');
	$pdf->SetXY(7.3,12.3);
	...
	$pdf->SetXY(4.4,14.7);
	$pdf->Cell(0,1,utf8_decode($row['prgDes']), 0, 1, 'J');
	...
	$pdf->Cell(0,.7,'(ANEXAR A ESTA CARATULA LA CUARTILLA DE ABAJO SELLADA)', 0, 1, 'C');
	...
	$pdf->Ln(.7);
	$pdf->SetFont('Arial','','12');
	$pdf->Cell(.8);$pdf->Cell(0,.7,'RESPONSABLE DEL PROGRAMA', 0, 1, 'J');
	$pdf->Cell(2.5);$pdf->Cell(0,.3,'EN LA EMPRESA', 0, 1, 'J');
	...
	$pdf->SetFont('Arial','','14');
	$pdf->Cell(0,.7,'3.-DETALLE MINUCIOSO DE LAS ACTIVIDADES REALIZADAS EN EL PERIODO', 0, 1, 'J');
	...
	$pdf->Line(1,26.7,20.6,26.7);
	$pdf->Line(1,26.8,20.6,26.8);
